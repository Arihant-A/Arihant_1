...
| `.lib` / `.json` | Liberty / JSON Netlist | Standard Cell Library & Netlist | The SKY130 `.lib` file contains the timing and logical definitions of real foundry cells (AND, OR, DFF, MUX, etc.). The output `.json` or `.v` netlist is the final, technology-mapped gate-level representation ready for Place-and-Route. |
